...
Note: in order to fit the tables into the page frame, some values were rounded up to one decimal point. However, the plots and final conclusions include more precise values.
...
\centering
    ...
    \caption{Comparison between the total distillate purity and total distillate mass results, respectively, of Method 1 and Method 2. Both methods provided a distillate of sufficient purity, therefore all batches can be used; plotting was not needed. Peaks can be seen when the reflux ratio is adjusted.}
...
Different stop values for the process were investigated, finally falling at 0.998 mol frac. water in the accumulator (water is more volatile than DMSO), as well as reflux values and internal pot pressure values. The reflux ratio was varied in the range R = 2 to R = 10, and the pressure was studied between 1 bar and 0.1 bar [cite].
...
\centering
    ...
    \caption{Distillate graphs demonstrating the effect of the reflux ratio, first parameter studied at 1 bar (at R = 2, 4, 6 and 9 bar, respectively).}
...
As we can see, with Method 1 our yields are still better than Method 2, and the new elapsed time is still inferior (119.55 h compared to 122.3 h). Therefore, even though it includes more steps, we can conclude that Method 1 is the most efficient.
...
The total volume to be evacuated consists of the vapor space in the feed tank and the internal volume of the distillation column. The feed tank has a total volume of 7.85 m³ and contains approximately 6 m³ of liquid before the start of the distillation, resulting in a vapor space of 1.85 m³. The distillation column has a height of 10 m and a diameter of 0.60 m, corresponding to an internal volume of approximately 2.83 m³. Accounting for additional piping and system volume, the total gas volume to be evacuated was approximated as 5.0 m³ [cite].
...
We want the pump-down time to be approximately 15 minutes:

$$
t = 15 \times 60 = 900\ \mathrm{s}
$$
...
The energy this pump's motor will require to operate for 15 minutes is 1.98 MJ, therefore it will not be accounted for in energy or cost estimations due to it being negligible.
