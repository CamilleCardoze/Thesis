# Nomenclature {-}

## Latin Symbols {- .unlisted}

| Symbol                | Meaning                                                            | Unit             |
| --------------------- | ------------------------------------------------------------------ | ---------------- |
| $A$                   | Area                                                               | m²               |
| $A_{calc}$            | Calculated heat-transfer area                                      | m²               |
| $A_{cond}$            | Condenser heat-transfer area                                       | m²               |
| $A_{tube}$            | Tube-side flow area                                                | m²               |
| $a$                   | Bundle-diameter correlation constant                               | -                |
| $b$                   | Bundle-diameter correlation exponent                               | -                |
| $B$                   | Batch number                                                       | -                |
| $B_{ij}$              | Binary interaction parameter from component $i$ to component $j$   | cal/mol          |
| $B_{ji}$              | Binary interaction parameter from component $j$ to component $i$   | cal/mol          |
| $C$                   | Cost                                                               | $                |
| $C_1$                 | Compressor 1                                                       | MJ/h             |
| $C_2$                 | Compressor 2                                                       | MJ/h             |
| $C_A$                 | Corrosion allowance                                                | mm or m          |
| $C_{shell}$           | Shell clearance                                                    | m                |
| $C_i$                 | Installed cost                                                     | $                |
| $C_p$                 | Purchase cost                                                      | $                |
| $C_{total}$           | Total installed cost                                               | $                |
| $C_{add}$             | Additional installed cost                                          | $                |
| $C_{pre,saving}$      | Pre-heater utility saving per batch                                | $/batch          |
| $C_{HP,steam\ saved}$ | Value of steam saved by the heat-pump method per batch             | $/batch          |
| $C_{HP,electricity}$  | Electricity cost of the heat-pump method per batch                 | $/batch          |
| $C_{HP,saving}$       | Net saving of the heat-pump method per batch                       | $/batch          |
| $c_p$                 | Specific heat capacity                                             | J/(kg·K)         |
| $D$                   | Diameter                                                           | m                |
| $D_{bundle}$          | Tube bundle diameter                                               | m                |
| $E$                   | Weld joint efficiency / Young’s modulus                            | - / Pa           |
| $E_{recovery}$        | Energy required for the recovery distillation step                 | GJ               |
| $E_{pre}$             | Heat recovered by the pre-heater per batch                         | GJ/batch         |
| $E_{HP,steam}$        | Steam energy replaced by the heat-pump method per batch            | GJ/batch         |
| $E_{HP,electricity}$  | Electricity energy required by the heat-pump compressors per batch | GJ/batch         |
| $F$                   | F-factor / LMTD correction factor                                  | $\sqrt{Pa}$ / -  |
| $F_c$                 | Cost correction factor                                             | -                |
| $F_t$                 | Compressor correction factor                                       | -                |
| $H$                   | Height                                                             | m                |
| $HETP$                | Height equivalent to one theoretical plate                         | m                |
| $HP$                  | Horsepower                                                         | hp               |
| $HP_1$                | Horsepower of compressor 1                                         | hp               |
| $HP_2$                | Horsepower of compressor 2                                         | hp               |
| $I$                   | First component in binary interaction table                        | -                |
| $J$                   | Second component in binary interaction table                       | -                |
| $k$                   | Overall heat-transfer coefficient                                  | W/(m²·K)         |
| $K_{vap}$             | Heat-transfer coefficient for vapour                               | W/(m²·K)         |
| $K_{dist}$            | Heat-transfer coefficient for distillate                           | W/(m²·K)         |
| $K_{calc}$            | Calculated heat-transfer coefficient                               | W/(m²·K)         |
| $L$                   | Length                                                             | m                |
| $L_{shell}$           | Shell length                                                       | m                |
| $m$                   | Mass                                                               | kg               |
| $m_{Total}$           | Total mass                                                         | kg               |
| $\dot{m}$             | Mass flow rate                                                     | kg/s or kg/h     |
| $MS$                  | Marshall and Swift index                                           | -                |
| $N$                   | Number of stages                                                   | -                |
| $N_t$                 | Number of tubes                                                    | -                |
| $N_{cycles}$          | Number of operating cycles                                         | -                |
| $n$                   | Amount of substance                                                | kmol             |
| $n_{IPA}$             | Amount of IPA                                                      | kmol             |
| $n_{H_2O}$            | Amount of water                                                    | kmol             |
| $Nu$                  | Nusselt number                                                     | -                |
| $Nu_w$                | Water-side Nusselt number                                          | -                |
| $P$                   | Pressure                                                           | Pa, bar, or mbar |
| $P_1$                 | Initial pressure                                                   | bar abs          |
| $P_2$                 | Final pressure                                                     | bar abs          |
| $p$                   | Pitch                                                              | m                |
| $p_t$                 | Tube pitch                                                         | m                |
| $Pr$                  | Prandtl number                                                     | -                |
| $Pr_w$                | Water-side Prandtl number                                          | -                |
| $Q$                   | Heat duty                                                          | W or MJ/h        |
| $Q_{max}$             | Maximum heat duty                                                  | W or MJ/h        |
| $Q_{reb}$             | Reboiler heat duty                                                 | W or MJ/h        |
| $Q_{feed}$            | Feed-heating energy                                                | W or MJ/h        |
| $Q_{total}$           | Total energy demand                                                | GJ               |
| $R$                   | Reflux ratio                                                       | -                |
| $R_f$                 | Fouling resistance                                                 | m²·K/W           |
| $Re$                  | Reynolds number                                                    | -                |
| $r$                   | Latent heat of vaporization                                        | J/kg             |
| $S$                   | Allowable stress / required pumping speed                          | Pa / m³/s        |
| $T$                   | Temperature                                                        | K or °C          |
| $\overline{T}$        | Average temperature                                                | K or °C          |
| $T_{1,1}$             | Hot-stream inlet temperature                                       | K or °C          |
| $T_{1,2}$             | Hot-stream outlet temperature                                      | K or °C          |
| $T_{2,1}$             | Cold-stream inlet temperature                                      | K or °C          |
| $T_{2,2}$             | Cold-stream outlet temperature                                     | K or °C          |
| $T_{comp.}$           | Compressor outlet temperature                                      | °C               |
| $T_{HX1}$             | Temperature after heat exchanger 1                                 | °C               |
| $t$                   | Time                                                               | h or s           |
| $V$                   | Volume                                                             | m³               |
| $\dot{V}$             | Vapour volumetric flow rate                                        | m³/s             |
| $V_{\text{max}}$      | Maximum required volume                                            | m³               |
| $V_{\text{tank}}$     | Selected tank volume                                               | m³               |
| $V_{packing}$         | Packing volume                                                     | m³               |
| $VF$                  | Vapour fraction                                                    | -                |
| $VF_{comp.}$          | Vapour fraction after compression                                  | -                |
| $VF_{HX1}$            | Vapour fraction after heat exchanger 1                             | -                |
| $v$                   | Velocity                                                           | m/s              |
| $x$                   | Liquid mole fraction / wall thickness                              | - / m or mm      |
| $x_{IPA}$             | IPA mole fraction                                                  | -                |
| $x_{H_2O}$            | Water mole fraction                                                | -                |
| $x_{DMSO}$            | DMSO mole fraction                                                 | -                |
| $x_{tube}$            | Tube wall thickness                                                | m                |
| $x_{shell}$           | Shell wall thickness                                               | m                |
| $y$                   | Vapour mole fraction                                               | -                |

## Greek Symbols {- .unlisted}

| Symbol             | Meaning                                     | Unit       |
| ------------------ | ------------------------------------------- | ---------- |
| $\alpha$           | Binary interaction parameter in NRTL table  | Cal/mol    |
| $\alpha_{cond}$    | Condensation-side heat-transfer coefficient | W/(m²·K)   |
| $\alpha_w$         | Water-side heat-transfer coefficient        | W/(m²·K)   |
| $\alpha_{boiling}$ | Boiling-side heat-transfer coefficient      | W/(m²·K)   |
| $\alpha_{shell}$   | Shell-side heat-transfer coefficient        | W/(m²·K)   |
| $\alpha_{st}$      | Steam condensation coefficient              | W/(m²·K)   |
| $\Delta P$         | Pressure jump or pressure drop              | Pa or mbar |
| $\Delta P_{total}$ | Total pressure jump across packed section   | mbar       |
| $\Delta T$         | Temperature difference                      | K or °C    |
| $\Delta T_1$       | First terminal temperature difference       | K          |
| $\Delta T_2$       | Second terminal temperature difference      | K          |
| $\Delta T_{lm}$    | Logarithmic mean temperature difference     | K          |
| $\Delta T_m$       | Mean temperature difference                 | K          |
| $\Delta T_{log}$   | Logarithmic mean temperature difference     | K          |
| $\Delta C$         | Difference in capital cost                  | $          |
| $\lambda$          | Thermal conductivity                        | W/(m·K)    |
| $\mu$              | Dynamic viscosity                           | Pa·s       |
| $\nu$              | Poisson’s ratio                             | -          |
| $\pi$              | Mathematical constant pi                    | -          |
| $\rho$             | Density                                     | kg/m³      |
| $\sigma$           | Surface tension                             | N/m        |
| $\infty$           | Infinite reflux condition                   | -          |

## Subscripts, indices and Conventions {- .unlisted}

| Symbol / convention | Meaning                                |
| ------------------- | -------------------------------------- |
| $i$                 | First component index                  |
| $j$                 | Second component index                 |
| $1$                 | First point, side, or terminal value   |
| $2$                 | Second point, side, or terminal value  |
| $1,1$               | Hot-stream inlet                       |
| $1,2$               | Hot-stream outlet                      |
| $2,1$               | Cold-stream inlet                      |
| $2,2$               | Cold-stream outlet                     |
| `in`                | Inlet or inside basis                  |
| `out`               | Outlet or outside basis                |
| `avg`               | Average value                          |
| `real`              | Real or estimated value                |
| `calc`              | Calculated value                       |
| `clean`             | Clean condition without fouling        |
| `dirty`             | Dirty condition including fouling      |
| `cond`              | Condensation or condenser side         |
| `boiling`           | Boiling side                           |
| `shell`             | Shell side or shell geometry           |
| `tube`              | Tube side or tube geometry             |
| `wall`              | Heat-transfer wall                     |
| `film`              | Film condition                         |
| `w`                 | Water or cooling water                 |
| `st`                | Steam                                  |
| `l`                 | Liquid phase                           |
| `v`                 | Vapour phase                           |
| `IPA`               | Isopropyl alcohol or isopropanol       |
| `H₂O`               | Water                                  |
| `DMSO`              | Dimethyl sulfoxide                     |
| `RO`                | Run-off stream                         |
| `Chg.`              | Charge                                 |
| `Dist.`             | Distillate                             |
| `L`                 | Liquid stream                          |
| `V`                 | Vapour stream                          |
| `top`               | Column top                             |
| `bottom`            | Column bottom                          |
| `external`          | External pressure condition            |
| `cr`                | Critical value                         |
| `max`               | Maximum value                          |
| `final`             | Final selected value                   |
| `lm`                | Logarithmic mean                       |
| `log`               | Logarithmic mean                       |
| `m`                 | Mean or corrected mean                 |
| `reb`               | Reboiler                               |
| `main`              | Main IPA distillation step             |
| `feed`              | Relating to the feed                   |
| `process`           | Complete process                       |
| `total`             | Total value                            |
| `recovery`          | Recovery step                          |
| `purchase`          | Purchase cost                          |
| `random`            | Random packing                         |
| `unit`              | Unit cost                              |
| `packing`           | Packing                                |
| `base`              | Base distillation system               |
| `pre`               | Pre-heater                             |
| `HP`                | Heat-pump method                       |
| `add`               | Additional cost                        |
| `HX`                | Heat exchanger                         |
| `HPS`               | High-pressure steam                    |
| `comp`              | Compressor                             |
| `int`               | Inter-cooler                           |
| `tank`              | Tank                                   |
| `col`               | Column                                 |
| `bundle`            | Tube bundle                            |
| $\dot{(\ )}$        | Flow rate or rate with respect to time |
| $\overline{(\ )}$   | Average value                          |
| `[-]`               | Dimensionless value                    |
| `≤`                 | Less than or equal to                  |
| `≥`                 | Greater than or equal to               |
| `AEL`               | TEMA exchanger type code               |
| `BKU`               | TEMA exchanger type code               |

## Abbreviations {- .unlisted}

| Abbreviation | Meaning                                     |
| ------------ | ------------------------------------------- |
| IPA          | Isopropyl alcohol / isopropanol             |
| DMSO         | Dimethyl sulfoxide                          |
| H₂O          | Water                                       |
| NRTL         | Non-random two-liquid model                 |
| BIP          | Binary interaction parameter(s)             |
| VLE          | Vapour-liquid equilibrium                   |
| HETP         | Height equivalent to one theoretical plate  |
| LMTD         | Logarithmic mean temperature difference     |
| RO           | Run-off                                     |
| Chg.         | Charge                                      |
| Dist.        | Distillate                                  |
| HX           | Heat exchanger                              |
| HX1          | Heat exchanger 1                            |
| HX2          | Heat exchanger 2                            |
| IC           | Inter-cooler                                |
| VF           | Vapour fraction                             |
| HP           | Horsepower                                  |
| HPS          | High-pressure steam                         |
| MS           | Marshall and Swift index                    |
| TEMA         | Tubular Exchanger Manufacturers Association |
| MJ           | Megajoule                                   |
| GJ           | Gigajoule                                   |
| kW           | Kilowatt                                    |
| W            | Watt                                        |
| Pa           | Pascal                                      |
| mbar         | Millibar                                    |
| bar abs      | Absolute bar pressure                       |
| mol%         | Mole percent                                |
| mass%        | Mass percent                                |
| kmol         | Kilomole                                    |
| kg/h         | Kilogram per hour                           |
| m³/h         | Cubic meter per hour                        |
| W/m²K        | Watt per square meter kelvin                |
| m²K/W        | Square meter kelvin per watt                |
| Cal/mol      | Calorie per mole                            |
| $/GJ         | Dollars per gigajoule                       |

\newpage
\pagenumbering{arabic}
